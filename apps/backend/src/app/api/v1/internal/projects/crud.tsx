...
const ownerPacks = [
  new Set(["c2c03bd1-5cbe-4493-8e3f-17d1e2d7ca43", "60b859bf-e148-4eff-9985-fe6e31c58a2a", "1343e3e7-dd7a-44a1-8752-701c0881da72"]),
];

export const internalProjectsCrudHandlers = createLazyProxy(() =>
  createCrudHandlers(internalProjectsCrud, {
      ...
      const result = await prismaClient.$transaction(async (tx) => {
                  ...
                  ? {
                      create: {
                        ...
                        standardEmailServiceConfig:
                          data.config.email_config.type === "standard"
                            ? {
                                create: {
        ...
        await tx.permission.create({
          data: {
            ...
            parentEdges: {
              createMany: {
                data: (["UPDATE_TEAM", "DELETE_TEAM", "READ_MEMBERS", "REMOVE_MEMBERS", "INVITE_MEMBERS"] as const).map((p) => ({
                  parentTeamSystemPermission: p,
                })),
              },
            },
        ...
        for (const userId of userIds) {
          ...
          if (!projectUserTx) {
            if (userId === user.id) {
              ...
              captureError(
                "project-creation-owner-packs",
                new StackAssertionError(
                  `User ${userId} in owner pack not found. The user that created this project is in an owner pack, but no user with that ID was found. Did they delete their account? Continuing silently, but you should probably update the owner pack.`,
                  { userId, creator: user },
                ),
              );
              continue;
          ...
          await tx.projectUser.update({
            ...
            data: {
              serverMetadata: {
                ...(serverMetadataTx ?? {}),
                managedProjectIds: [...(serverMetadataTx?.managedProjectIds ?? []), project.id],
              },
            },
          });
        }
