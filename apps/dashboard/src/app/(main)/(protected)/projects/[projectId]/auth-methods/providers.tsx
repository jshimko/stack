...
export const providerFormSchema = yup.object({
  ...
  clientId: yup.string().when("shared", {
    is: false,
    then: (schema) => schema.required(),
    otherwise: (schema) => schema.optional(),
  }),
  clientSecret: yup.string().when("shared", {
    is: false,
    then: (schema) => schema.required(),
    otherwise: (schema) => schema.optional(),
  }),
  ...
  return (
    <FormDialog
      ...
      render={(form) => (
        <>
          {hasSharedKeys ? (
            <SwitchField control={form.control} name="shared" label="Shared keys" />
          ) : (
          ...
          {form.watch("shared") ? (
            <Typography variant="secondary" type="footnote">
              Shared keys are created by the Stack team for development. It helps you get started, but will show a Stack logo and name on
              the OAuth screen. This should never be enabled in production.
            </Typography>
          ) : (
          ...
          {!form.watch("shared") && (
            <>
              <InputField control={form.control} name="clientId" label="Client ID" placeholder="Client ID" required />

              <InputField control={form.control} name="clientSecret" label="Client Secret" placeholder="Client Secret" required />
...
export function TurnOffProviderDialog(props: { open: boolean; onClose: () => void; onConfirm: () => void; providerId: string }) {
  return (
    <ActionDialog
    ...
    >
      <Typography>
        Disabling this provider will prevent users from signing in with it, including existing users who have used it before. They might not
        be able to log in anymore. Are you sure you want to do this?
      </Typography>
    </ActionDialog>
  );
}
        ...
        label={
          <div className="flex items-center gap-2">
            ...
            {isShared && enabled && (
              <SimpleTooltip
                tooltip={
                  "Shared keys are automatically created by Stack, but show Stack's logo on the OAuth sign-in page.\n\nYou should replace these before you go into production."
                }
              >
